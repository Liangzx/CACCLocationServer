...
public:
	...
	Dependent(const Dependent& rhs) : Person(rhs), education_(0) { education_ = (rhs.education_ == 0) ? 0 
...
int main(int, char*[])
{
	...
	vec.push_back("{hello:Cpp}");
	vec.push_back("{hello:Java}");
	writer.StartArray();
	for (size_t i = 0; i < vec.size(); i++)
	{
		writer.String(vec[i].c_str(), vec[i].length());
	}
	writer.EndArray();
